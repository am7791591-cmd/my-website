const state = {
  data: {
    stats: { students: 0, teachers: 0, classes: 0, presentToday: 0, collected: 0, totalDue: 0 },
    sessions: [],
    organization: { city: "Karachi" },
    classes: [],
    teachers: [],
    students: [],
    fees: [],
...
async function loadDashboard() {
  try {
    ...
    if (freshData) state.data = freshData;
  } catch (error) {
    console.error("Dashboard data load nahi ho saka, default values use ho rahi hain:", error);
  }
  render();
}
...
function applyRole() {
  ...
  if (roleHint) roleHint.textContent = hints[state.role] || "";
...
function renderStats() {
  if (!state.data.stats || !state.data.sessions || !state.data.organization) return;
  
  const stats = [
    ["Students", state.data.stats.students || 0],
    ["Teachers", state.data.stats.teachers || 0],
    ["Classes", state.data.stats.classes || 0],
    ["Present Today", state.data.stats.presentToday || 0],
    ["Active Sessions", state.data.sessions.filter(item => item.status === "active").length || 0],
    ["Branch", state.data.organization.city || "Karachi"]
  ];

  if (state.role === "admin") {
    stats.splice(3, 0, ["Fee Collected", money(state.data.stats.collected || 0)], ["Total Due", money(state.data.stats.totalDue || 0)]);
  }
...
function renderOverview() {
  ...
  if (classList && state.data.classes) {
...
function renderTeacherControls() {
  ...
  const activeClassLabel = $("#activeClassLabel");
  const topicInput = $("#topicInput");

  if (teacherSelect && state.data.teachers) {
  ...
  if (teacherClasses.length > 0 && !teacherClasses.some(item => item.id === state.selectedClassId)) {
  ...
  if (activeClassLabel) activeClassLabel.textContent = className(state.selectedClassId);
  if (topicInput) topicInput.disabled = state.role !== "admin";
}
...
function renderAttendance() {
  ...
  if (teacherAttendance && state.data.attendance) {
...
function renderFees() {
  const feeSearch = $("#feeSearch");
  const feeTable = $("#feeTable");
  if (!feeTable || !state.data.fees) return;

  const query = feeSearch ? feeSearch.value.trim().toLowerCase() : "";
  ...
  feeTable.innerHTML = rows.map(fee => {
    const due = fee.amount - fee.paid;
    const studentObj = state.data.students?.find(item => item.id === fee.studentId);
    return `
      <tr>
        <td><strong>${studentName(fee.studentId)}</strong><div class="meta">${className(studentObj?.classId)}</div></td>
        <td>${fee.month}</td>
...
function renderRecords() {
  const teacherRecords = $("#teacherRecords");
  if (teacherRecords && state.data.teachers) {
  ...
  if (studentRecords && state.data.students) {
...
function renderManageOptions() {
  const manageClassTeacher = $("#manageClassTeacher");
  if (manageClassTeacher && state.data.teachers) {
    manageClassTeacher.innerHTML = state.data.teachers.map(teacher => `
      <option value="${teacher.id}">${teacher.name} - ${teacher.subject}</option>
    `).join("");
  }

  const manageStudentClass = $("#manageStudentClass");
  if (manageStudentClass && state.data.classes) {
    manageStudentClass.innerHTML = state.data.classes.map(item => `
      <option value="${item.id}">${item.name}</option>
    `).join("");
  }

  const manageFeeStudent = $("#manageFeeStudent");
  if (manageFeeStudent && state.data.students) {
...
const beginClassBtn = $("#beginClassBtn");
if (beginClassBtn) beginClassBtn.addEventListener("click", () => beginClass().catch(error => showToast(error.message)));

const saveAttendanceBtn = $("#saveAttendanceBtn");
if (saveAttendanceBtn) saveAttendanceBtn.addEventListener("click", () => saveAttendance().catch(error => showToast(error.message)));

const teacherForm = $("#teacherForm");
if (teacherForm) teacherForm.addEventListener("submit", event => submitAdminForm(event, "/api/teachers").catch(error => showToast(error.message)));

const classForm = $("#classForm");
if (classForm) classForm.addEventListener("submit", event => submitAdminForm(event, "/api/classes").catch(error => showToast(error.message)));

const studentForm = $("#studentForm");
if (studentForm) studentForm.addEventListener("submit", event => submitAdminForm(event, "/api/students").catch(error => showToast(error.message)));

const newFeeForm = $("#newFeeForm");
if (newFeeForm) newFeeForm.addEventListener("submit", event => submitAdminForm(event, "/api/fees").catch(error => showToast(error.message)));

const refreshBtn = $("#refreshBtn");
if (refreshBtn) refreshBtn.addEventListener("click", () => loadDashboard().then(() => showToast("Data refreshed")));

const feeSearch = $("#feeSearch");
if (feeSearch) feeSearch.addEventListener("input", renderFees);

const clearFeeSearch = $("#clearFeeSearch");
if (clearFeeSearch) {
